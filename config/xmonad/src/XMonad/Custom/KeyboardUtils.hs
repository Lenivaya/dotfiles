...
wrapKbdLayout
  ...
  -> X ()  -- ^ An XMonad action with modified keyboard layout
wrapKbdLayout action = getCurrentLayout >>= \currentLayout ->
...
selectKbdLayout conf = switchToDefault >> getKbdLayouts >>= \layouts ->
