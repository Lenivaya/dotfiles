...
def find_binaries(binaries: List[str]) -> Union[Tuple[str, str], Tuple[None, None]]:
...
def get_decompression_command(
    archive_name: str, flags: list, to_dir: str = None
) -> List[str]:
    ...
    if (
        search(tar_full, archive_name) is not None
        or search(tar_short, archive_name) is not None
    ):
